...
**One item on the checklist has numbers, because I failed it myself.** Before running an experiment that looks for degradation, check that every criterion fails at least sometimes under the easiest condition; a criterion that passes 100% at baseline cannot register anything getting worse. In my own instruction-load run, two of three criteria never failed once across 180 observations — which is why that experiment returned a null, and why the null says something about the instrument rather than about the claim. `examples/measure_can_fail` runs the check over the committed data.
**Evidence.** *Observation per item, except the baseline-headroom check, which is measured: n = 180 observations, unit = one reply, three mechanical criteria, two of which never failed. No other figures are quoted: the aggregation case is the only one that has any, and a pass rate means nothing without its denominator and its exclusion rule beside it.* **This is a discipline, not a discovery** — the individual items appear as bullets in vendor evaluation guides. What I am contributing is the checklist and the insistence that it runs first. **One limitation to state plainly:** choosing the unit of analysis *after* seeing which one revealed the effect is outcome-driven re-analysis, whatever the merits of the choice. State the unit before the round.
