...
**On evidence.** This came out of establishing what a cross-bot memory layer could and couldn't be asked to do on one platform: real memory, scoped per user, holding goals and commitments, with entries written by asynchronous extraction and fetched by a similarity search the model chooses to call. There was no boundary category, and a prohibition stored as a fact behaved the way this note describes. One platform, one investigation, no counts — and the design rule is the part I'd defend, not a number.
